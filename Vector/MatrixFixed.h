...
{
	return imat3x2(2/(xmax-xmin),            0,-idiv((xmax+xmin),(xmax-xmin)),
	                          0,2/(ymax-ymin),-idiv((ymax+ymin),(ymax-ymin)));
}
...
{
	return imat4x3(2/(xmax-xmin),            0,             0,-idiv((xmax+xmin),(xmax-xmin)),
	                          0,2/(ymax-ymin),             0,-idiv((ymax+ymin),(ymax-ymin)),
				              0,            0,-2/(zmax-zmin),-idiv((zmax+zmin),(zmax-zmin)));
...
{
	int32_t f=F(1)/itan(imul(fovx,M_PI)/180/2);
...
{
	int32_t f=F(1)/itan(imul(fovy,M_PI)/180/2);
...
{
	int32_t f=F(1)/itan(imul(fov,M_PI)/180/2);
...
static inline bool imat2x2equal(imat2x2_t a,imat2x2_t b) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(a.m[i]!=b.m[i]) return false; return true; }
static inline bool imat3x2equal(imat3x2_t a,imat3x2_t b) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(a.m[i]!=b.m[i]) return false; return true; }
static inline bool imat3x3equal(imat3x3_t a,imat3x3_t b) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(a.m[i]!=b.m[i]) return false; return true; }
static inline bool imat4x3equal(imat4x3_t a,imat4x3_t b) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(a.m[i]!=b.m[i]) return false; return true; }
static inline bool imat4x4equal(imat4x4_t a,imat4x4_t b) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(a.m[i]!=b.m[i]) return false; return true; }

static inline bool imat2x2almostequal(imat2x2_t a,imat2x2_t b,int32_t epsilon) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(fabsf(a.m[i]-b.m[i])<epsilon) return false; return true; }
static inline bool imat3x2almostequal(imat3x2_t a,imat3x2_t b,int32_t epsilon) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(fabsf(a.m[i]-b.m[i])<epsilon) return false; return true; }
static inline bool imat3x3almostequal(imat3x3_t a,imat3x3_t b,int32_t epsilon) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(fabsf(a.m[i]-b.m[i])<epsilon) return false; return true; }
static inline bool imat4x3almostequal(imat4x3_t a,imat4x3_t b,int32_t epsilon) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(fabsf(a.m[i]-b.m[i])<epsilon) return false; return true; }
static inline bool imat4x4almostequal(imat4x4_t a,imat4x4_t b,int32_t epsilon) { for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) if(fabsf(a.m[i]-b.m[i])<epsilon) return false; return true; }
...
static inline imat2x2_t imat2x2smul(imat2x2_t a,int32_t b) { imat2x2_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=imul(a.m[i],b); return res; }
static inline imat3x2_t imat3x2smul(imat2x2_t a,int32_t b) { imat3x2_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=imul(a.m[i],b); return res; }
static inline imat3x3_t imat3x3smul(imat3x3_t a,int32_t b) { imat3x3_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=imul(a.m[i],b); return res; }
static inline imat4x3_t imat4x3smul(imat2x2_t a,int32_t b) { imat4x3_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=imul(a.m[i],b); return res; }
static inline imat4x4_t imat4x4smul(imat4x4_t a,int32_t b) { imat4x4_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=imul(a.m[i],b); return res; }

static inline imat2x2_t imat2x2sdiv(imat2x2_t a,int32_t b) { imat2x2_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=idiv(a.m[i],b); return res; }
static inline imat3x2_t imat3x2sdiv(imat2x2_t a,int32_t b) { imat3x2_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=idiv(a.m[i],b); return res; }
static inline imat3x3_t imat3x3sdiv(imat3x3_t a,int32_t b) { imat3x3_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=idiv(a.m[i],b); return res; }
static inline imat4x3_t imat4x3sdiv(imat2x2_t a,int32_t b) { imat4x3_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=idiv(a.m[i],b); return res; }
static inline imat4x4_t imat4x4sdiv(imat4x4_t a,int32_t b) { imat4x4_t res; for(int i=0;idiv(i<sizeof(a.m),sizeof(a.m[0]));i++) res.m[i]=idiv(a.m[i],b); return res; }
...
{
	return imul(imat2x2_11(m),imat2x2_22(m))-imul(imat2x2_21(m),imat2x2_12(m));
}

static inline int32_t imat3x3det(imat3x3_t m)
{
	return imul(imul(imat3x3_11(m),imat3x3_22(m)),imat3x3_33(m))
	      -imul(imul(imat3x3_11(m),imat3x3_32(m)),imat3x3_23(m))
	      +imul(imul(imat3x3_21(m),imat3x3_32(m)),imat3x3_13(m))
	      -imul(imul(imat3x3_21(m),imat3x3_12(m)),imat3x3_33(m))
	      +imul(imul(imat3x3_31(m),imat3x3_12(m)),imat3x3_23(m))
	      -imul(imul(imat3x3_31(m),imat3x3_22(m)),imat3x3_13(m));
}
...
{
	return imul(imat3x2_11(m),imat3x2_22(m))-imul(imat3x2_21(m),imat3x2_12(m));
}

static inline int32_t imat3x3affinedet(imat3x3_t m)
{
	return imul(imat3x3_11(m),imat3x3_22(m))-imul(imat3x3_21(m),imat3x3_12(m));
}

static inline int32_t imat4x3affinedet(imat4x3_t m)
{
	return imul(imul(imat4x3_11(m),imat4x3_22(m)),imat4x3_33(m))
	      -imul(imul(imat4x3_11(m),imat4x3_32(m)),imat4x3_23(m))
	      +imul(imul(imat4x3_21(m),imat4x3_32(m)),imat4x3_13(m))
	      -imul(imul(imat4x3_21(m),imat4x3_12(m)),imat4x3_33(m))
	      +imul(imul(imat4x3_31(m),imat4x3_12(m)),imat4x3_23(m))
	      -imul(imul(imat4x3_31(m),imat4x3_22(m)),imat4x3_13(m));
}

static inline int32_t imat4x4affinedet(imat4x4_t m)
{
	return imul(imul(imat4x4_11(m),imat4x4_22(m)),imat4x4_33(m))
	      -imul(imul(imat4x4_11(m),imat4x4_32(m)),imat4x4_23(m))
	      +imul(imul(imat4x4_21(m),imat4x4_32(m)),imat4x4_13(m))
	      -imul(imul(imat4x4_21(m),imat4x4_12(m)),imat4x4_33(m))
	      +imul(imul(imat4x4_31(m),imat4x4_12(m)),imat4x4_23(m))
	      -imul(imul(imat4x4_31(m),imat4x4_22(m)),imat4x4_13(m));
}
...
static inline imat2x2_t imat2x2inverse(imat2x2_t m)
{
	...
	return imat2x2( idiv(imat2x2_22(m),det),-idiv(imat2x2_12(m),det),
	              -idiv(imat2x2_21(m),det), idiv(imat2x2_11(m),det));
}
...
{
	return ivec2(imul(v.x,imat2x2_11(m))+imul(v.y,imat2x2_12(m)),
	            imul(v.x,imat2x2_21(m))+imul(v.y,imat2x2_22(m)));
}

static inline ivec2_t imat3x2transform(imat3x2_t m,ivec2_t v)
{
	return ivec2(imul(v.x,imat3x2_11(m))+imul(v.y,imat3x2_12(m))+imat3x2_13(m),
	            imul(v.x,imat3x2_21(m))+imul(v.y,imat3x2_22(m))+imat3x2_23(m));
}
