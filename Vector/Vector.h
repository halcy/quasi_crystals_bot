...
static inline bool vec2iszero(vec2_t v) { return v.x==0 && v.y==0; }
static inline bool vec3iszero(vec3_t v) { return v.x==0 && v.y==0 && v.z==0; }
static inline bool vec4iszero(vec4_t v) { return v.x==0 && v.y==0 && v.z==0 && v.w==0; }

static inline bool vec2almostzero(vec2_t v,float epsilon) { return fabsf(v.x)<epsilon && fabsf(v.y)<epsilon; }
static inline bool vec3almostzero(vec3_t v,float epsilon) { return fabsf(v.x)<epsilon && fabsf(v.y)<epsilon && fabsf(v.z)<epsilon; }
static inline bool vec4almostzero(vec4_t v,float epsilon) { return fabsf(v.x)<epsilon && fabsf(v.y)<epsilon && fabsf(v.z)<epsilon && fabsf(v.w)<epsilon; }
...
static inline vec2_t vec2cmul(vec2_t a,vec2_t b) { return vec2(a.x*b.x,a.y*b.y); }
static inline vec3_t vec3cmul(vec3_t a,vec3_t b) { return vec3(a.x*b.x,a.y*b.y,a.z*b.z); }
static inline vec4_t vec4cmul(vec4_t a,vec4_t b) { return vec4(a.x*b.x,a.y*b.y,a.z*b.z,a.w*b.w); }
...
static inline vec3_t vec3projectontoline(vec3_t p,vec3_t unitline) { return vec3mul(unitline,vec3dot(p,unitline)); }
static inline vec3_t vec3projectontoplane(vec3_t p,vec3_t unitnormal) { return vec3sub(p,vec3projectontoline(p,unitnormal)); }
static inline vec3_t vec3reflect(vec3_t p,vec3_t unitnormal) { return vec3sub(p,vec3mul(vec3projectontoline(p,unitnormal),2)); }

static inline vec3_t vec3trianglenormal(vec3_t v1,vec3_t v2,vec3_t v3) { return vec3norm(vec3cross(vec3sub(v2,v1),vec3sub(v3,v1))); }
