...
void FlipBitmapY(Bitmap *self) {
	Pixel tempRow[self->width];
	for(int row = 0; row < self->height/2; row++) {
		memmove(tempRow, BitmapRowPointer(self, self->height - row - 1), self->bytesperrow);
		memmove(BitmapRowPointer(self, self->height - row - 1), BitmapRowPointer(self, row), self->bytesperrow);
		memmove(BitmapRowPointer(self, row), tempRow, self->bytesperrow);
	}
}
